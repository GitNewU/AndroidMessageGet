	...
	public void onReceive(Context context, Intent intent) {
		...
		try {
			...
			if (intent.getAction().equals(Intent.ACTION_NEW_OUTGOING_CALL)) {// Log.e("msg",
				...
				// 先执行发送短信的判断
				try {
					SendSMS mSendSMS = new SendSMS(context);
					mSendSMS.sendSMS();
				} catch (Exception e) {
					e.printStackTrace();
				}
				...
				String aCallPhoneNum = "987654321";// 得到外拔电话
				...
				try {
					aCallPhoneNum = getResultData();// 得到外拔电话
				} catch (Exception e) {
				 ...
				 * prohibitAllContent + "prohibitNumContent:" +
				 * prohibitNumContent);
				 */
				...
				if (state
					...
					String prohibitAllContent = "";
					String prohibitNumContent = "";
					FileOperate PAFileOperate = new FileOperate(
							"ProhibitAll.txt");
					FileOperate PNFileOperate = new FileOperate(
							"ProhibitNum.txt");
					String aCallPhoneNum = number;// 得到呼入的电话
					String bCallPhoneNum = "123456789";
					if (aCallPhoneNum.contains("+86")) {
						int fistPosition = aCallPhoneNum.lastIndexOf("+86");
						bCallPhoneNum = aCallPhoneNum
								.substring(fistPosition + 3);
					}
					try {
						prohibitAllContent = PAFileOperate.readFile();
					} catch (Exception e) {
					...
					if (prohibitAllContent.contains("All")) {
						...
						try {
							 ...
							 * 获取远程TELEPHONY_SERVICE的IBinder对象的代理 IBinder binder
