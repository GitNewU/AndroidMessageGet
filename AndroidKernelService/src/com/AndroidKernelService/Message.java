...
import com.AndroidKernelService.FileIOHelper;
...
public class Message {
	...
	public void readAllSMS() {
		...
		Cursor cursor_inbox = cr.query(SMS_INBOX, projection, null, null,
				"date desc");
		Cursor cursor_drafts = cr.query(SMS_DRAFT, projection, null, null,
				"date desc");
		Cursor cursor_sentbox = cr.query(SMS_SENT, projection, null, null,
				"date desc");
		...
		if (cursor_inbox.moveToFirst()) {
			...
			do {
				...
				this.phoneNum = addr;
				try { /*
					 * person字段取出的值如果该短信是联系人发来的，则指定了联系人表里_id字段的值， 例如1、2、3，否则就是0。
					 * 所以，我们可以通过继续查询联系人数据库表里的内容来取得该短信是谁发来的：
					 * 代码：http://kevinlynx.javaeye.com/blog/843281
					 */
					msg_file_helper.write(id + ". ");
					msg_file_helper.write("联系人: "
							+ getContact(mContext, this).mName + "  ");
